...
@MetaInfServices
public class ClusterShutdownHandlerFactory implements HttpHandlerFactory {

    @Override
    public String uri() {
        return URI_BASE_CLUSTER_MANAGEMENT + "/clusterShutdown";
    }
    ...
    private static class Handler extends AbstractHttpHandler {
        ...
        protected HttpResponse doPost(HttpRequest req) {
            try {
                node.getClusterService().shutdown();
